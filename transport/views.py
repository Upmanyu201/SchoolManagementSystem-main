...
from django.core.paginator import Paginator
from users.decorators import module_required
# Make sure these imports point to your actual transport models location
from .models import Route, Stoppage, TransportAssignment 

@module_required('transport', 'view')
def transport_assignments_list(request):
    """
    Handles the display and filtering of all student transport assignments.
    """
    
    # 1. Get ALL filter parameters from URL
    search_query = request.GET.get('search_query', '').strip() 
    class_filter = request.GET.get('class_filter')       
    address_query = request.GET.get('address_filter', '').strip()
    route_filter = request.GET.get('route_filter')
    stoppage_filter = request.GET.get('stoppage_filter')

    # 2. Start with base QuerySet, optimizing for related data retrieval
    
    # FIX 1: Use Student.objects.all() instead of all_statuses() to ensure assignments
    # are found even if the student's status is non-default (e.g., graduated, left).
    student_qs = Student.objects.all().select_related('class_section') 

    student_prefetch = models.Prefetch(
        'student', 
        queryset=student_qs
    )
    
    # FIX 2: Ensure assignments only link to non-null students.
    queryset = TransportAssignment.objects.filter(student__isnull=False).prefetch_related(student_prefetch).select_related(
        'route', 
        'stoppage' 
    ).order_by('route__name', 'stoppage__name', 'student__first_name')
    
    # 3. Apply Filters (Existing logic is correct for conditional filtering)
    # Apply Search Query (Student Name/Admission Number)
    if search_query:
    ...
    # Apply Class Filter
    if class_filter:
        queryset = queryset.filter(student__class_section_id=class_filter)
        
    # Apply Address Filter
    if address_query:
        queryset = queryset.filter(student__address__icontains=address_query)

    # Apply Route Filter
    if route_filter:
        queryset = queryset.filter(route_id=route_filter)

    # Apply Stoppage Filter
    if stoppage_filter:
        queryset = queryset.filter(stoppage_id=stoppage_filter)


    # 4. Pagination
    page_number = request.GET.get('page', 1)
    paginator = Paginator(queryset, 25) # Display 25 assignments per page
    page_obj = paginator.get_page(page_number) # This contains the student data
    ...
    context = {
        ...
        'stoppages': Stoppage.objects.all().select_related('route').order_by('route__name', 'name'),
        
        # PASS CURRENT FILTER VALUES TO RETAIN FORM STATE
        'search_query': search_query,
        'class_filter': class_filter, 
        'current_address_filter': address_query,
        'current_route_filter': route_filter,
        'current_stoppage_filter': stoppage_filter,
    }
